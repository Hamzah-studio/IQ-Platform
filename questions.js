/**
 * مولد الأسئلة الديناميكي + بنك الأسئلة السيكومتري
 */
const QuestionBank = {
    // 1. مولد السلاسل الرقمية والرياضية
    generateNumericalSequence() {
        ...
        if (chosen === 'arithmetic') {
            ...
            // نمط متداخل (سلسلتان في سلسلة واحدة)
            const d1 = 2, d2 = -1;
            let base = 10;
    ...
    // 2. الأسئلة المباشرة عالية المستوى (المصفوفات والتناظر والمنطق الفضائي)
    staticQuestions: [
        {
    ...
    // 3. بناء مصفوفات بصرية ديناميكية عبر SVG
    ...
    // أخذ عينة مختلطة وموزونة للاختبار
    getTestSet(total = 10) {
        let testSet = [];
        // إضافة أسئلة خوارزمية ديناميكية
        for(let i = 0; i < Math.floor(total / 2); i++) {
            testSet.push(this.generateNumericalSequence());
        }
        // إضافة أسئلة منطقية ثابتة ومصفوفات بصرياً
